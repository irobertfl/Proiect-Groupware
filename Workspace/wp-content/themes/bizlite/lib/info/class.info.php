...
if ( ! class_exists( 'Bizlite_Info' ) ) {
    ...
    class Bizlite_Info {
         ...
         * Single instance.
         *
         * @var Bizlite_Info $instance Instance object.
         */
        ...
        public static function init( $config ) {
            if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Bizlite_Info ) ) {
                self::$instance = new Bizlite_Info;
        ...
        public function configure() {
            ...
            $this->menu_name     = isset( $this->config['menu_name'] ) ? $this->config['menu_name'] : sprintf( esc_html__( '%s info', 'bizlite' ), $this->theme_name );
            $this->page_name     = isset( $this->config['page_name'] ) ? $this->config['page_name'] : sprintf( esc_html__( '%s info', 'bizlite' ), $this->theme_name );
            $this->tabs          = isset( $this->config['tabs'] ) ? $this->config['tabs'] : array();
            $this->page_slug     = $this->theme_slug . '-details';
            $this->page_url     = admin_url( 'themes.php?page=' . $this->page_slug );
            $this->notice        = '<p>' . sprintf( esc_html__( 'Welcome! Thank you for choosing %1$s. To fully take advantage of the best our theme can offer please make sure you visit theme details page.', 'bizlite' ), esc_html( $this->theme_name ) ) . '</p><p><a href="' . esc_url( $this->page_url ) . '" class="button button-primary">' . sprintf( esc_html__( 'Get started with %1$s', 'bizlite' ), $this->theme_name ) . '</a><a href="#" class="btn-dismiss" data-userid="' . esc_attr( get_current_user_id() ) . '" data-nonce="' . esc_attr( wp_create_nonce( 'fino_dismiss_nonce' ) ) . '">' . esc_html__( 'Dismiss this notice', 'bizlite' ) . '</a></p>';
        }
        ...
        public function hooks() {
            ...
            // Dismiss AJAX.
            add_action( 'wp_ajax_fino_dismiss', array( $this, 'dismiss_callback' ) );
            add_action( 'wp_ajax_nopriv_fino_dismiss', array( $this, 'dismiss_callback' ) );
        }
        ...
        public function render_page() {
            ?>
            <div class="wrap about-wrap fpw-info-wrap">
                ...
                <?php endif; ?>
                <a href="<?php echo esc_url('http://freepsdworld.com'); ?>" target="_blank"><div class="wp-badge"></div></a>
        ...
        public function render_current_tab_content() {
            ...
            if ( method_exists( $this, $method ) ) {
                ...
                printf( esc_html__( '%s() method does not exist.', 'bizlite' ), $method );
            }
        }
        ...
        public function getting_started() {
            ...
            ?>
            <div class="feature-section fpw-section fpw-section-getting-started three-col">
                <?php foreach ( $content as $item ) : ?>
                    <?php $this->render_grid_item( $item ); ?>
                <?php endforeach; ?>
            </div><!-- .feature-section .fpw-section -->
            <?php
        }
        ...
        public function support() {
            ...
            ?>
            <div class="feature-section fpw-section fpw-section-support three-col">
                <?php foreach ( $content as $item ) : ?>
                    <?php $this->render_grid_item( $item ); ?>
                <?php endforeach; ?>
            </div><!-- .feature-section .fpw-section -->
        ...
        public function useful_plugins() {
            ...
            ?>
            <div class="fpw-section fpw-section-useful-plugins">
                ...
                <?php if ( true !== $tgmpa_complete ) : ?>
                    <a href="<?php echo esc_url( $tgmpa_url ); ?>" class="button button-primary"><?php esc_html_e( 'Manage Plugins', 'bizlite' ); ?></a>
                <?php endif; ?>
        ...
        public function free_vs_pro() {
            ...
            ?>
            <div class="fpw-section fpw-section-demo-content">
                <div class="compare-table table-responsive">
                    <table class="table">
                        <thead>
                        <tr>

                            <th class="db-bk-color-one"><?php _e('Product Features', 'bizlite'); ?></th>
                            <th class="db-bk-color-two"><?php _e('Free', 'bizlite'); ?></th>
                            <th class="db-bk-color-two"><?php _e('Premium', 'bizlite'); ?></th>
                        </tr>
                        </thead>
                        <tbody>
        ...
        public function upgrade_to_pro() {
            ...
            ?>
            <div class="fpw-section fpw-section-upgrade-to-pro">
        ...
        public function assets( $hook ) {
            ...
            if ( in_array( $hook, array( 'themes.php', 'appearance_page_' . $this->page_slug ), true ) ) {
                wp_enqueue_style( 'fino-info', get_template_directory_uri() . '/lib/info/css/info.css', array(), '1.0.0' );
                wp_enqueue_script( 'fino-info', get_template_directory_uri() . '/lib/info/js/info.js', array(), '1.0.0' );
            }
        ...
        public function display_admin_notice() {
            ...
            $dismiss_status = get_user_meta( $user_id, 'fino_dismiss_status', true );
            ?>
            <?php if ( current_user_can( 'edit_theme_options' ) && 'themes' === $screen_id && 1 !== absint( $dismiss_status ) ) : ?>
                <div class="fpw-info-notice notice notice-info">
        ...
        public function dismiss_callback() {
            ...
            if ( false === wp_verify_nonce( $wpnonce, 'fino_dismiss_nonce' ) ) {
                wp_send_json( $output );
            }

            update_user_meta( $userid, 'fino_dismiss_status', 1 );
